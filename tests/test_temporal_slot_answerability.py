"""Synthetic fixtures for 0.14.2 temporal mode, slot fit, and answerability."""
...
def test_exact_slot_has_strongest_structured_fit() -> None:
    ...
    assert "structured_adjustment=" not in matched.reason
    assert "slot_fit=0.67" in other_service_result.reason
    assert "structured_adjustment=" in other_service_result.reason
...
def test_support_episode_inherits_only_supported_slot_fit() -> None:
    ...
    assert "structured_adjustment=" in by_key["region-support"].reason
