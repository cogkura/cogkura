-- Cogkura semantic consolidation schema (version 003)

CREATE UNIQUE INDEX IF NOT EXISTS memories_tenant_id_memory_id_idx
    ON cogkura.memories (tenant_id, id);

CREATE TABLE IF NOT EXISTS cogkura.semantic_claims (
...
CREATE TABLE IF NOT EXISTS cogkura.memory_derivations (
    tenant_id TEXT NOT NULL,
    ...
    FOREIGN KEY (tenant_id, target_memory_id)
        REFERENCES cogkura.memories(tenant_id, id)
        ON DELETE CASCADE,
