...
class RetrievalContext:
    """Structured contextual cues supplied with a retrieval request."""
    ...
    @classmethod
    def from_canonical_dict(cls, payload: Mapping[str, Any] | None) -> RetrievalContext:
        """Hydrate from persisted JSON; missing payload yields empty context."""
        if not payload:
            return cls()
        raw_attributes = payload.get("attributes", {})
    ...
    def is_empty(self) -> bool:
        ...
        return self.to_canonical_dict() == RetrievalContext().to_canonical_dict()


def retrieval_contexts_equal(
    left: RetrievalContext | None,
    right: RetrievalContext | None,
) -> bool:
    left_context = left if left is not None else RetrievalContext()
    right_context = right if right is not None else RetrievalContext()
    return left_context.to_canonical_dict() == right_context.to_canonical_dict()
