-- Cognema observation storage schema (version 001)

CREATE SCHEMA IF NOT EXISTS cognema;

CREATE TABLE IF NOT EXISTS cognema.observations (
    id UUID PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS observations_tenant_id_idx
    ON cognema.observations (tenant_id);

CREATE INDEX IF NOT EXISTS observations_tenant_subject_idx
    ON cognema.observations (tenant_id, subject_id);
...
CREATE INDEX IF NOT EXISTS observations_tenant_event_type_idx
    ON cognema.observations (tenant_id, event_type);

CREATE INDEX IF NOT EXISTS observations_source_updated_at_idx
    ON cognema.observations (source_updated_at);

CREATE TABLE IF NOT EXISTS cognema.observation_revisions (
    id UUID PRIMARY KEY,
    observation_id UUID NOT NULL
        REFERENCES cognema.observations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS cognema.connector_checkpoints (
    tenant_id TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS cognema.memories (
    id UUID PRIMARY KEY,
    tenant_id TEXT NOT NULL,
    subject_id TEXT,
...
CREATE TABLE IF NOT EXISTS cognema.memory_evidence (
    memory_id UUID NOT NULL
        REFERENCES cognema.memories(id) ON DELETE CASCADE,
    observation_id UUID NOT NULL
        REFERENCES cognema.observations(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS cognema.schema_migrations (
