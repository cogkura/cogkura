...
Cogkura is in development. Through `0.17.0`, the library provides observational cue-competition diagnostics on `inspect_recall` (`CompetitionDiagnostics`, `CompetitionConfig`), and through `0.16.5`, the library provides encoding-context capture on observations and episodes (`ObservationContext`, `MemoryContextSignature`), retrieval-context matching (`RetrievalContext`, `ContextMatch`), episodic context reinstatement on declarative activation (`ContextReinstatement`, `ActivationConfig.context_reinstatement_weight`), semantic support-context propagation (`SemanticSupportContextEvidence`, `ActivationConfig.semantic_context_reinstatement_weight`), contextual observability on `inspect_recall` (`RetrievalContextDiagnostics`, `MemoryAssessment.context`), plus application integration via `Memory.process()`, `Memory.prepare_context()`, `MemoryContext`, and `Memory.record_context_use()`, observation ingestion, episodic encoding, semantic consolidation with temporal reconsolidation, ACT-R declarative activation with global eligible-candidate ranking, spreading activation, Ebbinghaus-inspired forgetting dynamics, bounded working-memory selection with precision-aware goal relevance, outcome-driven learning via `Memory.learn()`, and read-only metamemory assessment via `Memory.assess_memory()`, with explicit `record_access()` / `record_context_use()` reinforcement (presentation vs use), `apply_forgetting()` maintenance, simulated `as_of` on encode/consolidate/process, episode `valid_at` filtering, candidate-set IDF ranking, near-duplicate collapse, temporal current-state policy, soft entity slot admission, coverage-based accessibility with precision-aware ranking, conjunctive structured slot matching, positive bounded structured ranking, retrieval diagnostics with explicit eligibility and provenance, metamemory answerability, multi-entity conjunction, incident tag seeding, superseded-only SUPPORT exclusion, metamemory `MISSING_KNOWLEDGE`, working-memory same-slot collapse, evidence-chronology activation with `inspect_recall()`, lexical semantic slot matching for plain-language cues, bounded soft admission for long-horizon current facts, evidence-linked associative semantic reachability, authoritative current semantic admission, current-at-snapshot temporal semantics, one-hop entity associative reach, canonical retrieval features for semantic relevance, bounded evidence-to-evidence association, cardinality-aware recall identity, relevance-specificity competition among distinct semantics, contextual association with entity recovery and entity-indexed episode hops, structured entity relationships supplied via observation metadata and traversed at recall time, deterministic working-memory chunking with coverage-aware selection, and explicit semantic-primary correctness for `SEMANTIC_WITH_SUPPORT` chunks.

## Scope of 0.17.0

Implemented in `0.17.0`:

- `CompetitionDiagnostics` on `inspect_recall` candidates explaining pairwise cue rivalry (strength, direction, structural evidence);
- `CompetitionConfig` with conservative defaults; `enabled=False` preserves prior inspect behaviour;
- retrieval scoring, activation, admission, and ranking unchanged for identical inputs vs `0.16.5`.

Interference penalties and inhibition belong to later `0.17.x` releases.

## Scope of 0.16.5

Implemented in `0.16.5`:

- `CONTEXT_CONFLICT` metamemory and `NO_CONTEXTUAL_MATCH` when comparable evidence has zero positive matches;
- semantic support reason/count semantics (`NO_RETRIEVAL_CONTEXT`, `NO_COMPARABLE_CONTEXT`, `NOT_EVALUATED`);
- mixed semantic support `PARTIAL_CONTEXT_CONFLICT` without changing propagation formulae;
- frozen `concept_ids == ()` on default encoder; consolidated architecture docs and freeze tests;
- retrieval scoring unchanged for identical inputs vs `0.16.4` (diagnostic bug fixes only).

Following `0.16.5`, the 0.16 contextual-memory architecture is frozen.

## Scope of 0.16.4

Implemented in `0.16.4`:

- `DeterministicRetrievalContextPolicy` and `RetrievalContextDiagnostics` on `inspect_recall` (canonical) and additive `MemoryAssessment.context`;
- inspect-only threshold-crossing and pre/post context rank fields on `RecallInspectionCandidate`;
- `MetamemoryConfig.context_underspecified_margin` (default `0.0`; classification only);
- retrieval behaviour unchanged for identical inputs vs `0.16.3` (no-context and populated-context neutrality gates);
- behaviour tests in [`tests/test_016_4_context_observability.py`](tests/test_016_4_context_observability.py).

Contextual metamemory does not feed `MemoryContext.render()` or change `record_context_use` flattening.

## Scope of 0.16.3

Implemented in `0.16.3`:

- `DeterministicSemanticSupportContextPolicy` with `Rⱼ = Mⱼ × Vⱼ`, `Rₛ = ΣRⱼ/K`, `Cₛ = λsem × Rₛ`, `Aₛ' = Aₛ + Cₛ` on semantic activation;
- `ActivationConfig.semantic_context_reinstatement_weight` (default `0.25`; `0` disables contribution);
- `SemanticSupportContextEvidence` on `RetrievalDiagnostics` plus aggregate `ContextReinstatement` summary;
- no-context and semantic `weight=0` preserve `0.16.2` semantic retrieval behaviour; episodic `0.16.2` path unchanged;
- behaviour tests in [`tests/test_016_3_semantic_support_context.py`](tests/test_016_3_semantic_support_context.py).

Populated retrieval context may change semantic recall ranking and threshold outcomes when `semantic_context_reinstatement_weight > 0`. Working-memory render format (for no-context calls) and `record_context_use` flattening are unchanged.

## Scope of 0.16.2

Implemented in `0.16.2`:

- `DeterministicContextReinstatementPolicy` with `Rᵢ = Mᵢ × Vᵢ`, `Cᵢ = λctx × Rᵢ`, `Aᵢ' = Aᵢ + Cᵢ` on episodic activation;
- `ActivationConfig.context_reinstatement_weight` (default `0.50`; `0` disables contribution);
- `ContextReinstatement` diagnostics on `RetrievalDiagnostics` plus `activation_before_context`;
- no-context and `weight=0` preserve `0.16.1` retrieval behaviour;
- behaviour tests in [`tests/test_016_2_context_reinstatement.py`](tests/test_016_2_context_reinstatement.py).

Populated retrieval context may change recall ranking and threshold outcomes when `context_reinstatement_weight > 0`. Semantic memories and working-memory render format (for no-context calls) are unchanged.

## Scope of 0.16.1

Implemented in `0.16.1`:

- public `RetrievalContext` and optional `retrieval_context=` on recall APIs;
- `DeterministicContextMatcher` with exact normalised dimension comparison;
- ephemeral `ContextMatch` on `RetrievalDiagnostics` and `RecallInspectionResult.retrieval_context`;
- mandatory retrieval-neutrality regression tests;
- example [`examples/retrieval_context.py`](examples/retrieval_context.py).

Recall, activation, admission, ranking, and working-memory selection are unchanged when retrieval context is populated.

## Scope of 0.16.0

Implemented in `0.16.0`:

- public `ObservationContext` on `ObservationInput` and persisted observations;
- public `MemoryContextSignature` on episodic memories as `encoding_context`;
- deterministic episode aggregation from member observations plus derived subject, entity, and source fields;
- Postgres migration `009_encoding_context.sql`;
- mandatory retrieval-neutrality regression tests;
- example [`examples/encoding_context.py`](examples/encoding_context.py).

Retrieval, activation, admission, ranking, and working-memory selection are unchanged when encoding context is populated.

## Scope of 0.15.10

Implemented in `0.15.10`:

- explicit semantic structural primary for `SEMANTIC_WITH_SUPPORT` chunks, independent of member relevance ordering;
- early `InternalInvariantError` for malformed support chunks at construction time;
- regression tests in [`tests/test_working_memory_chunking.py`](tests/test_working_memory_chunking.py).

## Scope of 0.15.9

Implemented in `0.15.9`:

- deterministic chunk formation after recall (collections, semantic+support, episodic leftovers);
- coverage-aware greedy chunk selection with `max_items` as chunk capacity;
- `MemoryContext.render()` and `record_context_use()` over serialized chunk text and flattened member recalls;
- [`tests/test_working_memory_chunking.py`](tests/test_working_memory_chunking.py).
...
Implemented in `0.15.7`:

- candidate-set association indexes and bounded seed/neighbour expansion;
- entity recovery from seed text against known product entity ids;
- entity-indexed episode-to-episode hops and tightened contextual evidence overlap;
- inspectable association roles and path diagnostics;
- [`tests/test_contextual_association.py`](tests/test_contextual_association.py).

## Scope of 0.15.6

Implemented in `0.15.6`:

- cardinality-aware recall identity so many-valued semantics stay distinct at recall time;
- relevance specificity tiers that rank direct object matches above weaker evidence overlap;
- collapse and competition diagnostics on `inspect_recall()`;
- [`tests/test_semantic_competition.py`](tests/test_semantic_competition.py).

## Scope of 0.15.5

Implemented in `0.15.5`:

- canonical retrieval content features for semantic relevance (stopwords, morphology, contractions);
- asymmetric query-coverage relevance with inspectable matched features;
- bounded evidence-to-evidence association with `AssociationPath` diagnostics;
- [`tests/test_canonical_retrieval_features.py`](tests/test_canonical_retrieval_features.py) and [`tests/test_deterministic_semantic_relevance.py`](tests/test_deterministic_semantic_relevance.py).
...
- `0.15.10`: working-memory chunk primary correctness for `SEMANTIC_WITH_SUPPORT` (done).
- `0.15.9`: working-memory chunking and coverage-aware selection (done).
